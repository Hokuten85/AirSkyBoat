...
itemObject.onItemCheck = function(target)
    if target:getCharMod(xi.mod.EVASION) < 20 then
		return 0
	end
...
itemObject.onItemUse = function(target)
    ...
    if target:getCharMod(xi.mod.EVASION) < 20 then
        target:addCharMod(xi.mod.EVASION,1)
        if target:getCharMod(xi.mod.EVASION) == 20 then
            target:PrintToPlayer("You have reached the max enhancement for this stat. Additional usage of Sonia's Diary is allowed")
