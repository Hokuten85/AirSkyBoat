CREATE or replace TABLE single
...
create or replace table item_pricing (
	item_id int,
    price int,
	hqBonus int
);
...
delimiter //
CREATE or replace PROCEDURE determine_item_pricing(IN pItemId INT, OUT pPrice INT)
start_label: BEGIN
	DECLARE totalPrice INT DEFAULT 0;
    DECLARE done INT DEFAULT FALSE;
    DECLARE vID, vIngredient1, vIngredient2, vIngredient3, vIngredient4, vIngredient5, vIngredient6, vIngredient7, vIngredient8, vResult, vResultHQ1, vResultHQ2, vResultHQ3, vResultQty, vResultHQ1Qty, vResultHQ2Qty, vResultHQ3Qty, vRecipePrice, vitemPrice, vhqBonus, vPrice INT;
    ...
    IF(NOT EXISTS(SELECT 1 FROM item_pricing WHERE item_id = pItemId)) THEN
		IF(EXISTS(SELECT 1 FROM synth_recipes WHERE pItemId IN (Result,ResultHQ1,ResultHQ2,ResultHQ3))) THEN
			...
			SET vPrice = (SELECT price FROM vendor_prices WHERE itemid = pItemId);
		
			IF pPrice > vPrice THEN
				SET pPrice = vPrice;
			END IF;

            IF pPrice > 0 THEN
				INSERT INTO item_pricing VALUES (pItemId, pPrice, vhqBonus);
			ELSEIF (EXISTS(SELECT 1 FROM vendor_prices WHERE itemid = pItemId)) THEN
				SET pPrice = (SELECT price FROM vendor_prices WHERE itemid = pItemId);
			END IF;
        END IF;
    END IF;
END//

CREATE or replace PROCEDURE determine_equipment_pricing()
BEGIN
  DECLARE done INT DEFAULT FALSE;
  DECLARE vitemid INT;
  DECLARE vPrice INT;
