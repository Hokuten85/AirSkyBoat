    ...
    ************************************************************************/
                             ...
                             CBattleEntity* POriginalTarget, bool isCritical)
    {
        ...
        if (damage > 0)
        {
            ...
            if (giveTPtoAttacker)
            {
                ...
                PAttacker->addTP((int16)(tpMultiplier * ( ((isCritical ? 25 : 0) + baseTp) * (1.0f + 0.01f * (float)((PAttacker->getMod(Mod::STORETP) + getStoreTPbonusFromMerit(PAttacker)))))));
            }
                       ...
                       int8 offsetAccuracy) // subWeaponAttack is for calculating acc of dual wielded sub weapon
    {
        ...
        else
        {
            ...
            bool isH2H = targ_weapon->isHandToHand();

            if ((isOffhand || isTwoHanded) && !isH2H)
            {
                maxHitRate = 95;
            }
